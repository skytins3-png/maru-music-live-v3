...
## GitHub 빌드
이 압축 안의 파일을 저장소 최상단에 올린 뒤 Actions에서 `Build MARU MUSIC LIVE V3.1.2 APK`를 실행합니다. 이전 패치와 섞지 말고 이 전체 파일을 기준으로 사용합니다.
...
## 검증 범위
- `scripts/run_release_validation.sh`: 프로젝트 규칙, Java import, 핵심 자바 자체 테스트, 예외 처리 컴파일 테스트를 실행합니다.
...
## V3.1.2 저장소 잔여 파일 오류를 먼저 정리하는 방법
